...
public class ProviceInfoPlace extends LActivity {
	...
	AddressDb DBADDR=new AddressDb(this,null,null,2);
		...
		try {
			...
			if (code == 1) {
				...
				for (int i = 0; i < categoryList.length(); i++) {
					...
					if(parent==0){
						db.execSQL("INSERT INTO "+DBADDR.PROVINCE+"("+"_ID"+")VALUES"+"("+id+")");
						db.execSQL("INSERT INTO "+DBADDR.PROVINCE+"("+"parent"+")VALUES"+"("+parent+")");
						db.execSQL("INSERT INTO "+DBADDR.PROVINCE+"("+"name"+")VALUES"+"("+name+")");
					}
				}
				...
				for(int i=0;i<categoryList.length();i++){
					...
					int id = item.getInt("id");
					int parent = item.getInt("parent");
					String sql="SELECT * " + "FROM " + DBADDR.PROVINCE + " WHERE　_ID in ("+parent+")";
                    Cursor cursor=db.rawQuery(sql, null);
                    if(cursor!=null){
                        db.execSQL("INSERT INTO "+DBADDR.CITY+"("+"_ID"+")VALUES"+"("+id+")");
                        db.execSQL("INSERT INTO "+DBADDR.CITY+"("+"parent"+")VALUES"+"("+parent+")");
                        db.execSQL("INSERT INTO "+DBADDR.CITY+"("+"name"+")VALUES"+"("+name+")");
                    }
                    cursor.close();
                }
                ...
                for(int i=0;i<categoryList.length();i++){
                    ...
                    int id = item.getInt("id");
                    int parent = item.getInt("parent");
                    String sql="SELECT * "  + "FROM " + DBADDR.CITY + " WHERE　_ID in ("+parent+")";
                    Cursor cursor=db.rawQuery(sql, null);
                    if(cursor!=null){
                        db.execSQL("INSERT INTO "+DBADDR.COUNTRY+"("+"_ID"+")VALUES"+"("+id+")");
                        db.execSQL("INSERT INTO "+DBADDR.COUNTRY+"("+"parent"+")VALUES"+"("+parent+")");
                        db.execSQL("INSERT INTO "+DBADDR.COUNTRY+"("+"name"+")VALUES"+"("+name+")");
                    }
                    cursor.close();
                }
...
public void quer(){
    String sql="SELECT * " + "FROM " + DBADDR.PROVINCE + " WHERE　_ID in ("+0+")";
    Cursor cursor=db.rawQuery(sql, null);
